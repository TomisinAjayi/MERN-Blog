...
const BlogDetail = () => {
  ...
  useEffect(() => {
    fetchDetails().then((data)=> {
      setBlog(data.blog)
      setInputs({title:data.blog.title, descrition: data.blog.description})
    });
  }, [id]);
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <Box border={3} borderColor="green" borderRadius={10} boxShadow="10px 10px 20px #ccc" padding={3} margin={"auto"} mt={3} display="flex" flexDirection="column" width="80%">
